...
CREATE TABLE IF NOT EXISTS agreement_summary(
  ...
  decisions INTEGER, comparable INTEGER, no_gnn INTEGER, too_few INTEGER, no_scores INTEGER,
...
CREATE TABLE IF NOT EXISTS agreement_hist(
  bucket INTEGER PRIMARY KEY, lo REAL NOT NULL, hi REAL NOT NULL, decisions INTEGER NOT NULL);
"""
...
class _Summary(_Rollup):
    ...
    def step(self, src, an, lo, hi):
        ...
        counts = {r["status"]: int(r["n"]) for r in an.execute(
            "SELECT status, COUNT(*) n FROM model_agreement GROUP BY status")}
        rows = an.execute(
            "SELECT decision_id, rho, tau_b, rbo, top1_same, top3_overlap, top5_overlap,"
            " top10_overlap, n FROM model_agreement WHERE status='ok'"
        ...
        fell = int(an.execute("SELECT COUNT(*) FROM model_agreement WHERE fell_back=1"
                              ).fetchone()[0])
        an.execute(
            "INSERT INTO agreement_summary VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            ("all", time.time(), sum(counts.values()), len(rows),
             counts.get("no_gnn", 0), counts.get("too_few", 0), counts.get("no_scores", 0),
...
def _ambiguous(an) -> int:
    try:
        return int(an.execute(
            ...
            " WHERE a.status='ok' GROUP BY a.decision_id HAVING COUNT(*) > 1)"
        ).fetchone()[0])
    except Exception:
        return 0
...
CREATE TABLE IF NOT EXISTS agreement_series(
  axis TEXT NOT NULL, seq INTEGER NOT NULL, label TEXT,
...
class _Series(_Rollup):
    ...
    def step(self, src, an, lo, hi):
        ...
        rows = an.execute(
            ...
            " WHERE status='ok' ORDER BY decision_id").fetchall()
        size = bucket_size(len(rows))
        gate = min_decisions(size)
        out = []
        for seq, i in enumerate(range(0, len(rows), size)):
            chunk = rows[i:i + size]
            out.append(_point("window", seq, None, chunk, gate))
        for seq, g in enumerate(an.execute(
                "SELECT * FROM model_generations ORDER BY seg_from_ts")):
            chunk = an.execute(
                "SELECT decision_id, ts, rho, tau_b, rbo, top1_same FROM model_agreement"
                " WHERE status='ok' AND ts >= ? AND ts < ? ORDER BY decision_id",
                (g["seg_from_ts"], g["seg_to_ts"])).fetchall()
            p = _point("generation", seq, g["trial"], chunk, gate)
            ...
            out.append(p)
        an.executemany(
            "INSERT INTO agreement_series(axis, seq, label, from_decision, to_decision,"
            " from_ts, to_ts, decisions, rho_median, rho_mean, rho_q1, rho_q3, tau_mean,"
            " rbo_mean, same_top, gate, trial, generation, retrained, overlapped_by,"
            " bucket_decisions)"
            " VALUES(:axis,:seq,:label,:from_decision,:to_decision,:from_ts,:to_ts,"
            ":decisions,:rho_median,:rho_mean,:rho_q1,:rho_q3,:tau_mean,:rbo_mean,"
            ":same_top,:gate,:trial,:generation,:retrained,:overlapped_by,:bucket_decisions)",
            [dict(p, bucket_decisions=size) for p in out])
        return hi, len(out)


def _point(axis, seq, label, chunk, gate) -> dict:
    p = {"axis": axis, "seq": seq, "label": label, "trial": None, "generation": None,
         "retrained": None, "overlapped_by": None,
...
CREATE TABLE IF NOT EXISTS agreement_breakdown(
  dim TEXT NOT NULL, key TEXT NOT NULL, decisions INTEGER NOT NULL,
  rho_median REAL, rho_mean REAL, tau_mean REAL, rbo_mean REAL, same_top INTEGER,
  cat_rank REAL, cat_pct REAL, gnn_rank REAL, gnn_pct REAL, delta_pct REAL,
  fell_back INTEGER,
  PRIMARY KEY(dim, key));
"""
...
class _Breakdown(_Rollup):
    ...
    def step(self, src, an, lo, hi):
        ...
        for dim in DIMS:
            groups = {}
            for r in an.execute(
                    "SELECT %s k, rho, tau_b, rbo, top1_same, taken_cat_rank,"
                    " taken_cat_pct, taken_gnn_rank, taken_gnn_pct, fell_back"
                    " FROM model_agreement WHERE status='ok' AND %s IS NOT NULL" % (dim, dim)):
                groups.setdefault(r["k"], []).append(r)
            for k, rs in groups.items():
                m, mean, _, _ = _spread([r["rho"] for r in rs])
                cat_pct = _med([r["taken_cat_pct"] for r in rs])
                gnn_pct = _med([r["taken_gnn_pct"] for r in rs])
                out.append((
                    dim, str(k), len(rs), m, mean,
                    _mean([r["tau_b"] for r in rs]), _mean([r["rbo"] for r in rs]),
                    sum(1 for r in rs if r["top1_same"]),
                    _med([r["taken_cat_rank"] for r in rs]), cat_pct,
                    _med([r["taken_gnn_rank"] for r in rs]), gnn_pct,
                    (None if cat_pct is None or gnn_pct is None else gnn_pct - cat_pct),
                    sum(1 for r in rs if r["fell_back"])))
        an.executemany("INSERT INTO agreement_breakdown VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                       out)
        return hi, len(out)
