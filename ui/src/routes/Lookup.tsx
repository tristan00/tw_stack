...
const matchCols: Col<LookupCampaignRow>[] = [
...
export function Lookup() {
  ...
  const conditioned = params.getAll('c').length > 0
  ...
  if (!data) return null
  const shown = conditioned ? [...cols.slice(0, 4), ...matchCols, ...cols.slice(4)] : cols
  const tiles = [
    { label: 'campaigns', value: n(data.campaigns), sub: conditioned ? 'ever in a matching situation' : 'recorded' },
    ...(conditioned
  ...
  return (
    <div className="space-y-5">
      <ConditionBar />
      <div className={`grid gap-3 sm:grid-cols-2 ${conditioned ? 'lg:grid-cols-4' : 'lg:grid-cols-3'}`}>
        {tiles.map((t) => (
          <MetricTile key={t.label} metric={{ label: t.label, value: t.value, unit: null, sub: t.sub, state: 'neutral', spark: [] }} />
        ))}
      </div>
      <Section title={conditioned ? 'matching campaigns' : 'campaigns'} scope={data.scope}>
        <DataTable
          rows={data.rows ?? []}
          cols={shown}
