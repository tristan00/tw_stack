...
NEW_COLUMNS = (
    ...
    "near_%s_%d_%s" % (g, k + 1, f) for g in sorted(F.NEAR_FIELDS)
    for k in range(F.NEAR_K.get(g, F.NEAR_K_DEFAULT)) for f in F.NEAR_FIELDS[g])
...
def _run():
    ...
    check(atk["opt_tgt_enemy_1_dist"] == 4.0
          ...
          and atk["opt_tgt_enemy_1_hp"] == 6.0,
